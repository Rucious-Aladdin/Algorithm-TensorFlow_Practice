...
if __name__ == "__main__":
    ...
    for depth in range(5, 15):            
        ...
        clf = DecisionTreeClassifier(max_depth=depth, min_samples_split=10, min_samples_leaf=5)
        
        if time_check:
            import time
            x = time.time()
            print("starting_time: " + str(x))
            clf.fit(x_train, t_train)
            print(str(time.time() - x))
            time_list.append(time.time() - x)
        else:
            clf.fit(x_train, t_train)
...
# hyper parameter tuning 수행
# maxdepth에 대한 hyper parameter tuning 수행
# 결과: [0.6352, 0.643, 0.645, 0.6407, 0.6401, 0.6401, 0.6401, 0.6401, 0.6401, 0.6401]
"""
10 9 5
30 8 6
5 13 14
