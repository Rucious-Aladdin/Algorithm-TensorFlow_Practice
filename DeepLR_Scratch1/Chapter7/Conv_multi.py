...
class ConvMulti:
    def __init__(self, input_dim = (1, 28, 28),
                 conv_param = {"filter_num":30, "filter_size":5,
                               "pad":0, "stride":1, "filter2_num":30, "filter2_size":3},
        ...
        conv2_output_size = ((conv_output_size / 2) - filter2_size + 2*filter_pad) / \
                            filter_stride + 1
        pool2_output_size  = int(filter2_num * (conv2_output_size / 2) * 
                                 (conv2_output_size / 2))
        ...
        self.params["W2"] = weight_init_std * \
                            np.random.randn(filter2_num, filter_num,
                                            filter2_size, filter2_size) 
        self.params["b2"] = np.zeros(filter2_num)
        
        ## Affine Network
        self.params["W3"] = weight_init_std * \
                            np.random.randn(pool2_output_size, 
                                            hidden_size)
        ...
        self.layers["Affine1"] = Affine(self.params["W3"],
                                        self.params["b3"])
        self.layers["Relu2"] = Relu()
        self.layers["Affine2"] = Affine(self.params["W4"],
                                        self.params["b4"])
